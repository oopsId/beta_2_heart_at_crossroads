...
(() => {
    const economy = window.heartEconomy;
    if (!economy) throw new Error('heartEconomy config must load before diamonds.js');
    ...
    function writeBank(value) {
        ...
        const normalized = Number.isFinite(numeric) && numeric >= 0 ? Math.floor(numeric) : INITIAL_DIAMONDS;
        try { localStorage.setItem(storageKey(BANK_KEY), String(normalized)); }
        catch (error) { console.error('[economy] Failed to persist diamond bank:', error); }
        return normalized;
    }

    const baseCreateFreshRunStats = createFreshRunStats;
    createFreshRunStats = function stage2dCreateFreshRunStats(...args) {
    ...
    if (existingBank === null) existingBank = writeBank(legacyRunBalance() ?? INITIAL_DIAMONDS);
    if (!runtimeActive) {
    ...
    loadSession = async function stage2dLoadSession(...args) {
        const ok = await baseLoadSession(...args);
        if (!ok) return ok;
        const bank = readBank();
        stats.diamonds = bank ?? writeBank(stats.diamonds);
        updateDiamondsDisplay();
        return ok;
    };

    const baseSaveSession = saveSession;
    saveSession = async function stage2dSaveSession(...args) {
        writeBank(stats.diamonds);
        return await baseSaveSession(...args);
    };

    let pendingCompletion = null;
    const baseShowEpilogue = showEpilogue;
    showEpilogue = function stage2dShowEpilogue(...args) {
    ...
    saveProfile = async function stage2dSaveProfile(...args) {
    ...
    window.stage2dEconomy = economy;
    window.stage2dReadDiamondBank = readBank;
    window.stage2dWriteDiamondBank = writeBank;
})();
