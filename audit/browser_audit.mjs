...
async function instrument(page, bag){
  ...
  page.on('requestfailed',r=>bag.failedRequests.push({url:r.url(),error:r.failure()?.errorText}));
}

// A. Ordinary browser with the page exactly as shipped, including Telegram's script.
const bagA={consoleErrors:[],pageErrors:[],failedRequests:[],requests:[]};
const ctxA=await browser.newContext({viewport:{width:1280,height:900}}); const pageA=await ctxA.newPage(); await instrument(pageA,bagA);
await pageA.goto(url,{waitUntil:'domcontentloaded'});
const telegramProbe=await pageA.evaluate(()=>({
  runtimeIsTelegram: (()=>{try{return eval('isTelegram')}catch(e){return null}})(),
  hasTelegram:!!window.Telegram,
  hasWebApp:!!window.Telegram?.WebApp,
  initData:window.Telegram?.WebApp?.initData||'',
  initDataUnsafe:window.Telegram?.WebApp?.initDataUnsafe||null,
  version:window.Telegram?.WebApp?.version||null,
  userId:(()=>{try{return eval('userId')}catch(e){return null}})()
}));
check('ordinary browser is not misclassified as Telegram Mini App',telegramProbe.runtimeIsTelegram===false,telegramProbe);
if(telegramProbe.runtimeIsTelegram===true && !telegramProbe.initData){
  add('P0','FALSE_TELEGRAM_DETECTION','Ordinary browser is classified as Telegram Mini App',JSON.stringify(telegramProbe),'Persistence selects Telegram CloudStorage even without authenticated Telegram init data.');
}
const storageProbeA=await pageA.evaluate(async()=>{
  localStorage.clear(); window._inMemoryStorage={};
  saveToStorage('audit_probe','persist-me'); await new Promise(r=>setTimeout(r,200));
  return {local:localStorage.getItem('audit_probe'),memory:window._inMemoryStorage?.audit_probe||null};
});
check('ordinary browser persistence lands in localStorage',storageProbeA.local==='persist-me',storageProbeA);
if(storageProbeA.local!=='persist-me') add('P0','BROWSER_PERSISTENCE_FALLBACK','Ordinary browser persistence does not reach localStorage',JSON.stringify(storageProbeA),'Reload can erase game state; current CloudStorage synchronous errors fall into in-memory storage.');
await ctxA.close();

// B. Browser-only mode: block Telegram bridge so the intended non-Telegram path can be tested independently.
const bagB={consoleErrors:[],pageErrors:[],failedRequests:[],requests:[]};
const ctxB=await browser.newContext({viewport:{width:1280,height:900}}); const page=await ctxB.newPage(); await instrument(page,bagB);
await page.route('https://telegram.org/js/telegram-web-app.js',route=>route.abort());
await page.goto(url,{waitUntil:'domcontentloaded'});
await page.evaluate(()=>{localStorage.clear();localStorage.setItem('tempAccessGranted','true');});
await page.reload({waitUntil:'domcontentloaded'});
await page.click('#start-game');
await page.waitForTimeout(3000);
const originalPathRequests=bagB.requests.filter(u=>u.includes('/heart_at_crossroads/'));
...
if(originalPathRequests.length) add('P0','RUNTIME_BASE_PATH_CROSSLOAD','beta_2 runtime requests the original /heart_at_crossroads path',`${originalPathRequests.length} requests observed; e.g. ${originalPathRequests.slice(0,6).join(', ')}`,'beta_2 tests can execute original assets/data or fail when original Pages path is unavailable.');

// Save contract under known-good localStorage backend.
let saveSnapshot;
try {
  ...
  check('saveSession writes current state in browser mode',parsed?.currentChapter===3&&parsed?.currentScene===7,saveSnapshot);
  check('saveSession writes key read by loadSession',!!saveSnapshot.last_session,saveSnapshot);
  if(saveSnapshot.gameSession&&!saveSnapshot.last_session) add('P0','RUNTIME_SAVE_KEY_MISMATCH','Runtime save writes gameSession while load path expects last_session',JSON.stringify(saveSnapshot),'Continue cannot restore current saves.');
}catch(e){add('P0','RUNTIME_SAVE_CRASH','saveSession runtime test crashed',String(e),'Persistence cannot be trusted.');}

try {
  const result=await page.evaluate(async()=>{
    currentChapter=1;currentScene=0;stats.heart=0;choices=[];localStorage.removeItem('last_session');await loadSession();
...
try {
  const result=await page.evaluate(async()=>{
    ...
    currentChapter=1;currentScene=0;stats.heart=0;choices=[];await loadSession();
...
try {
  ...
  if(result!==true) add('P0','RUNTIME_LONELY_UNREACHABLE','lonely_path requirement evaluates false even with crown=100, heart=1, leaf=1',`checkRequirements returned ${result}`,'The lonely ending is unreachable through its declared rule.');
}catch(e){add('P0','RUNTIME_REQUIREMENT_CRASH','Ending requirement evaluation crashed',String(e),'Ending routing is broken.');}

try {
 const result=await page.evaluate(async()=>{
   window.pendingEndingId='silence_with_mark';currentChapter=10;currentScene=7;saveSession();await new Promise(r=>setTimeout(r,200));
...
try {
 const result=await page.evaluate(()=>{currentChapter=6;currentScene=3;stats.heart=8;resetGameState();return {currentChapter,currentScene,heart:stats.heart,diamonds:stats.diamonds,completionCount:stats.completionCount};});
...
try {
 const result=await page.evaluate(async()=>{
   const oldLoad=loadChapter,oldShow=showScene,oldPre=preloadAssets;let shown=0;
   preloadAssets=async()=>({success:0,failed:0});loadChapter=async()=>false;showScene=async()=>{shown++};
   try{await startGame();}catch(e){};loadChapter=oldLoad;showScene=oldShow;preloadAssets=oldPre;document.getElementById('loading-overlay')?.remove();return {shown};
 });
...
await page.screenshot({path:`${outDir}/browser-start.png`,fullPage:true}); await ctxB.close(); await browser.close();
...
const report={url,summary,checks,findings,modes:{ordinaryBrowser:bagA,browserOnly:bagB}};
...
md+=`\n## Runtime errors\n\nordinaryBrowser consoleErrors=${JSON.stringify(bagA.consoleErrors.slice(0,30))}\n\nbrowserOnly pageErrors=${JSON.stringify(bagB.pageErrors)}\n\nbrowserOnly consoleErrors=${JSON.stringify(bagB.consoleErrors.slice(0,30))}\n\nbrowserOnly failedRequests=${JSON.stringify(bagB.failedRequests.slice(0,30))}\n`;
fs.writeFileSync(`${outDir}/browser-audit.md`,md);console.log(JSON.stringify(summary));for(const f of findings)console.log(`${f.severity} ${f.code}: ${f.title}`);
