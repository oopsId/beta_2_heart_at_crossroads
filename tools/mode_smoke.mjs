...
const normalState = await normal.evaluate(async () => {
  ...
  return { mode: window.heartDevMode, control: !!document.getElementById('stage0k-dev-replay-control'), forced: stage0kDevForceFirstPlaythrough(), replayTextPresent: Object.prototype.hasOwnProperty.call(scene, 'second_playthrough_text') };
});
...
const specialState = await special.evaluate(() => {
  const enabled = stage0kSetDevForceFirstPlaythrough(true);
  return { mode: window.heartDevMode, control: !!document.getElementById('stage0k-dev-replay-control'), enabled, forced: stage0kDevForceFirstPlaythrough() };
});
if (!specialState.mode || !specialState.control || !specialState.enabled || !specialState.forced) throw new Error(`developer mode did not activate override: ${JSON.stringify(specialState)}`);
