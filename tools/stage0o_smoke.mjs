...
const phone=await page.evaluate(async()=>{
  ...
  document.getElementById('game-container').style.display='block';document.querySelector('.dialogue-box').style.display='flex';document.body.classList.add('stage0j-compose-scene');
  ...
  try{typeText=(text,el,cb)=>{el.textContent=text;cb?.();return true};playSound=()=>null;playMusic=()=>null;showSceneWithTimer(scene,generation);await new Promise(r=>setTimeout(r,180));
    const p=document.getElementById('phone-compose-overlay').getBoundingClientRect(),t=document.getElementById('dialogue-text').getBoundingClientRect();return{top:p.top,bottom:p.bottom,height:p.height,textTop:t.top,gap:t.top-p.bottom};
...
assert(phone.top<=6,'Phone is not at top edge',JSON.stringify(phone));assert(phone.bottom<phone.textTop&&phone.gap>=8,'Phone still covers readable text',JSON.stringify(phone));
