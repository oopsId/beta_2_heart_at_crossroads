...
html = (ROOT / 'heart_at_crossroads.html').read_bytes().decode('utf-8')
chapter10 = json.loads((ROOT / 'assets/data/chapter10.json').read_bytes().decode('utf-8'))
finals = json.loads((ROOT / 'assets/data/finals.json').read_bytes().decode('utf-8'))
...
load_block = html[html.index('async function loadFinals'):html.index('function showEnding')]
assert 'showStartScreen();' not in load_block, 'loadFinals still dumps failures to start screen'
